...
YUMS=/data/xcat/node-setup/node-files/yums-to-install.txt
YUMS_LOGIN=/data/xcat/node-setup/node-files/yums-login-to-install.txt
...
echo "---------------------------------------------"   &>> $LOG
yum groupinstall -y "Compute Node"                     &>> $LOG
yum groupinstall -y "Development tools"                &>> $LOG
yum groupinstall -y "Additional Development"           &>> $LOG
yum groupinstall -y "Compatibility libraries"          &>> $LOG
...
do
    ...
    echo "--------------------------------------------------------------------------" &>> $LOG
    /usr/bin/yum -y install --skip-broken "$package"                             | tee -a $LOG
done
