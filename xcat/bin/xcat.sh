...
NODE=none
FORCE=no
uuid_set_wait_time=1 # 60 seconds
discovery_time=25
GROUP=compute
#group defaults
_OSIMAGE=centos6.9
...
function show_menus() {
   ...
   # mode-specific help
      ...
      echo "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~"
      echo "Usage: /data/xcat/bin/xcat.sh -p -n node [-f] [-g group]"
      ...
      echo "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~"
      echo "Usage: /data/xcat/bin/xcat.sh -d -n node"
      echo " -n|--node                node name"
   elif [[ "$mode" == "reimage" ]]; then
      echo "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~"
      echo "Usage: /data/xcat/bin/xcat.sh -r -n node"
      echo " -n|--node                node name"
   elif [[ "$mode" == "adopt" ]]; then
      echo "Usage: /data/xcat/bin/xcat.sh -o -n node -j ip [-g group]"
   elif [[ "$mode" == "clear" ]]; then
      echo "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~"
      echo "Usage: /data/xcat/bin/xcat.sh -c"
   elif [[ "$mode" == "addgroup" ]]; then
      echo "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~"
      echo "Usage: /data/xcat/bin/xcat.sh -a -g group [options]"
      ...
      echo "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~"
      echo "Usage: /data/xcat/bin/xcat.sh -k -g group"
      echo "-g|--group                Group name"
   elif [[ "$mode" == "listgroup" ]]; then
      echo "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~"
      echo "Usage: /data/xcat/bin/xcat.sh -l"
   fi
...
_first_uuid_line=`cat $_discovery_file | egrep -v UUID | grep undef | head -n1`
_uuid=`echo "$_first_uuid_line" | awk '{print $1}'`
_node_state=`echo "$_first_uuid_line" | awk '{print $2}'`
   if [[ -s $_discovery_file && $_node_state =~ "undef" ]]; then
      echo ""
      print_lines
      echo "Looks like we have a undef UUID, processing it from:"
      echo "$_first_uuid_line"
      print_lines
   fi
...
do
      ...
      if [ -z "$_uuid" ]; then  
            ...
            echo "UUID: $_uuid"
            #break
            echo "Allowing 60 secs for the MAC address to be discovered"
            #sleep 60
            echo ""
            countdown_timer
            echo ""
...
function adopt_node () {
echo $ipaddr
logger nodeadd $NODE groups=$GROUP hosts.ip=$ipaddr
IPMI_ADDR="10.3"${ipaddr#"10.1"}
